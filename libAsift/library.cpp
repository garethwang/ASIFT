...
void compute_gradient_orientation(float* igray,float *grad, float *ori, int width, int height)
{
    ...
    for (r = 0; r < rows; r++)
      for (c = 0; c < cols; c++) {
        if (c == 0)
          xgrad = 2.f * (igray[r*cols+c+1] - igray[r*cols+c]);
        else if (c == cols-1)
          xgrad = 2.f * (igray[r*cols+c] - igray[r*cols+c-1]);
        else
          xgrad = igray[r*cols+c+1] - igray[r*cols+c-1];
        if (r == 0)
          ygrad = 2.f * (igray[r*cols+c] - igray[(r+1)*cols+c]);
        else if (r == rows-1)
          ygrad = 2.f * (igray[(r-1)*cols+c] - igray[r*cols+c]);
        else
...
{
	for(int i=width*height-1; i>0; i--)	out[i] = (red[i] + green[i] + blue[i]) /3.f;
}
