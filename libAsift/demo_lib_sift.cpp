...
void default_sift_parameters(siftPar &par)
{
	...
	par.BorderDist = 5; 
	par.Scales = 3;
	par.PeakThresh = 255.f * 0.04f / 3.f;
...
//	par.MatchRatio = 0.75; // Guoshen Yu. Since l1 distance is used for matching instead of l2, a larger threshold is needed. 
	par.MatchRatio = 0.73f; // Guoshen Yu. Since l1 distance is used for matching instead of l2, a larger threshold is needed. 
...
//		octSize *= 0.5;
		...
		printf("Double image size not allowed. Guoshen Yu\n");
		exit(-1);
...
int NotOnEdge(flimage& dog, int r, int c, float octSize,siftPar &par)
{
	...
	float	H00 = dog(c,r-1) - 2.f * dog(c,r) + dog(c,r+1), /* AMIR: div by ? */
		H11 = dog(c-1,r) - 2.f * dog(c,r) + dog(c+1,r),
		H01 = ( (dog(c+1,r+1) - dog(c-1,r+1)) - (dog(c+1,r-1) - dog(c-1,r-1)) ) / 4.f;
	...
	float octSize, keypointslist& keys, int movesRemain,siftPar &par)
{
	...
	*/
	...
	float octScale = par.InitSigma * pow(2.f, (s + offset[0]) / (float) par.Scales);
...
float FitQuadratic(float offset[3], flimage* dogs, int s, int r, int c)
{
	...
	g[0] = ((*dog2)(c,r) - (*dog0)(c,r)) / 2.f;
	g[1] = ((*dog1)(c, r + 1) - (*dog1)(c, r - 1)) / 2.f;
	g[2] = ((*dog1)(c + 1, r) - (*dog1)(c - 1, r)) / 2.f;

	/* Fill in the values of the Hessian from pixel differences. */
	H[0][0] = (*dog0)(c,r)   - 2.f * (*dog1)(c,r) + (*dog2)(c,r);
	H[1][1] = (*dog1)(c,r-1) - 2.f * (*dog1)(c,r) + (*dog1)(c,r+1);
	H[2][2] = (*dog1)(c-1,r) - 2.f * (*dog1)(c,r) + (*dog1)(c+1,r);
	H[0][1] = H[1][0] = ( ((*dog2)(c,r+1) - (*dog2)(c,r-1)) -
					((*dog0)(c,r+1) - (*dog0)(c,r-1)) ) / 4.f;
	H[0][2] = H[2][0] = ( ((*dog2)(c+1,r) - (*dog2)(c-1,r)) -
					((*dog0)(c+1,r) - (*dog0)(c-1,r)) ) / 4.f;
	H[1][2] = H[2][1] = ( ((*dog1)(c+1,r+1) - (*dog1)(c-1,r+1)) -
					((*dog1)(c+1,r-1) - (*dog1)(c-1,r-1)) ) / 4.f;
...
// 	printf("\n");
	...
	*/
	...
	return ((*dog1)(c,r) + 0.5f * (solution[0]*g[0]+solution[1]*g[1]+solution[2]*g[2]));
}
...
//	float	hist[par.OriBins], distsq, dif, gval, weight, angle, interp;
	...
	for (int i = 0; i < par.OriBins; i++) {
		...
		if (	hist[i] > hist[prev]  &&  hist[i] > hist[next]  &&
			...
			angle = 2.f * PI * (i + 0.5f + interp) / par.OriBins - PI;
...
void SmoothHistogram(float* hist, int bins)
{
	...
	for (int i = 0; i < bins; i++) {
		temp = hist[i];
		hist[i] = ( prev + hist[i] + hist[(i + 1 == bins) ? 0 : i + 1] ) / 3.f;
		prev = temp;
	}
}
	...
	float scale, float row, float col,siftPar &par)
{
	...
	*/
	...
	int intval;
	for (int i = 0; i < VecLength; i++) {
		intval =  (int)(512.f * key.vec[i]);
		key.vec[i] = (float) MIN(255, intval);
	}
}
	...
	const flimage& grad, const flimage& ori, float scale, float row, float col,siftPar &par)
{
	...
	float	radius = 1.414f * spacing * (IndexSize + 1) / 2.f;
	...
	for (int i = -iradius; i <= iradius; i++) {
		for (int j = -iradius; j <= iradius; j++) {
			...
			*/
			 ...
			 rx = rpos + IndexSize / 2.f - 0.5f;
			 cx = cpos + IndexSize / 2.f - 0.5f;
	
			/* Test whether this sample falls within boundary of index patch. */
			if (	rx > -1.f && rx < (float) IndexSize  &&
				cx > -1.f && cx < (float) IndexSize )
				AddSample(
					index, key, grad, ori,
					float(irow + i), float(icol + j), rpos, cpos, rx, cx,par);
		}
	}
}
	...
	float r, float c, float rpos, float cpos, float rx, float cx,siftPar &par)
{
	...
	float	sigma  = par.IndexSigma * 0.5f * IndexSize,
		weight = exp(- (rpos * rpos + cpos * cpos) / (2.f * sigma * sigma)),
//		mag    = weight *  grad(c,r); 
...
//	float	ori = orim(c,r) -  key.angle; 
	...
	if (par.IgnoreGradSign) {
		while (ori > PI ) ori -= PI;
		while (ori < 0.f) ori += PI;
	} else {
		while (ori > 2.f*PI) ori -= 2.f*PI;
		while (ori < 0.f   ) ori += 2.f*PI;
	}
	...
	float mag, float ori, float rx, float cx,siftPar &par)
{
	...
	float	oval = OriSize * ori / (par.IgnoreGradSign ? PI : 2.f*PI);
...
//		oi = (oval >= 0.0) ? oval : oval - 1.0;
	...
	for (int r = 0; r < 2; r++) {
		...
		if (rindex >=0 && rindex < IndexSize) {
			rweight = mag * ((r == 0) ? 1.f - rfrac : rfrac);

			for (int c = 0; c < 2; c++) {
				cindex = ci + c;
				if (cindex >=0 && cindex < IndexSize) {
					cweight = rweight * ((c == 0) ? 1.f - cfrac : cfrac);
					ivec = index[rindex][cindex];
					for (orr = 0; orr < 2; orr++) {
						oindex = oi + orr;
						if (oindex >= OriSize)  /* Orientation wraps around at PI. */
							oindex = 0;
						oweight = cweight * ((orr == 0) ? 1.f - ofrac : ofrac);
						ivec[oindex] += oweight;
					}
				}
			}
