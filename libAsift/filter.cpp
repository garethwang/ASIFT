...
float * directional_gauss_filter(float xsigma, float ysigma, float angle, int *kwidth, int *kheight)
{
	...
	for(int y = -l2; y <= l2; y++)
		for(int x = -l2; x <= l2; x++)
		{
			...
			float a = (float) angle * PI / 180.f;
			float sina = sinf(a);
			float cosa = cosf(a);
			
			float ax = (float) x * cosa + (float) y * sina;
			float ay = -(float) x * sina + (float) y * cosa;
			kernel[(y+l2) * ksize + x + l2] =  expf(-(ax*ax)/(2.f*xsigma2)  - (ay*ay)/(2.f*ysigma2) );  
	...
	//		float distmin = MAXFLOAT;
			...
			for(int i=-bloc;i<=bloc;i++)
				for(int j=-bloc;j<=bloc;j++)
					...
					if (dist < distmin) {distmin=dist;green=(float)valueg;}
			  ...
			  for (j=0; j< height ;j++) 
				{
				  ...
				  laplacian[j*width + i] =  - 4.f  * input[width*j+i] + input[width*j+im]+ input[width*j+i1]+input[width*jm + i] + input[width*j1 + i];
			     }
		}
